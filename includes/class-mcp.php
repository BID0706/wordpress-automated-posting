...
class ILLE_PG_MCP {
    ...
    private function tool_definitions(): array {
        return [
            ...
            [
                ...
                'inputSchema' => [
                    ...
                    'properties' => [
                        ...
                        'featured_image_gen' => [ 'type' => 'boolean', 'description' => 'Generate and attach an AI featured image', 'default' => true ],
                    ],
                ],
            ],
            ...
            [
                'name'        => 'create_post',
                'description' => 'Publish pre-written content directly to WordPress. Use this when you have already drafted the post. QUALITY STANDARD — your draft must satisfy all of the following before calling this tool: (1) Title: 50–60 characters, benefit-driven, contains the focus keyphrase near the start. (2) Content: 800+ words; opens with an intro paragraph that includes the focus keyphrase; at least 3 <h2> sections, 2 of which contain the focus keyphrase; ends with a conclusion; includes at least 1 internal link and 1 outbound link; keyphrase appears naturally 4–8 times (0.5–3% density) — do not stuff. (3) focus_keyword: always provide — if the site runs Yoast SEO this maps to the Yoast focus keyphrase and drives the green-light analysis; on sites without Yoast it is still stored as post meta and used for discoverability. (4) excerpt: always provide a 120–156 character meta description that includes the focus keyphrase and compels a click — Yoast flags anything under 120 characters as too short. (5) Featured image: leave featured_image_gen enabled unless the user supplies their own image via upload_image. When publish is false the post is saved as a draft; use publish_post to publish it after review.',
                'inputSchema' => [
                    'type'       => 'object',
                    'required'   => [ 'title', 'content' ],
                    'properties' => [
                        'title'          => [ 'type' => 'string',  'description' => 'Post title — 50–60 characters, benefit-driven, focus keyphrase appears at or near the start. Example: "7 Proven Ways to Cut Your Grocery Bill by 30%"' ],
                        'content'        => [ 'type' => 'string',  'description' => 'Post body as HTML. 800+ words. Structure: (a) intro paragraph containing the focus keyphrase, (b) at least 3 <h2> sections with the keyphrase in at least 2 of them, (c) 1+ internal links to related posts/pages, (d) 1+ outbound links to authoritative sources, (e) conclusion paragraph. Use <p>, <h2>, <h3>, <ul>, <li>, <strong>, <em>. Keyphrase density 0.5–3% — do not over-repeat.' ],
                        'focus_keyword'  => [ 'type' => 'string',  'description' => 'STRONGLY RECOMMENDED. The primary search keyphrase this post targets (e.g. "how to save money on groceries"). If the site uses Yoast SEO this drives the full Yoast green-light analysis (keyphrase in title, intro, subheadings, slug, meta description, density). On any site it is stored as post meta for discoverability. Must appear in the title, intro, at least 2 subheadings, and the excerpt.' ],
                        'excerpt'        => [ 'type' => 'string',  'description' => 'STRONGLY RECOMMENDED. Meta description shown in search results. 120–156 characters (Yoast flags under 120 as too short). Must include the focus keyphrase and give readers a clear reason to click. Example: "Discover 7 simple strategies to cut your grocery bill by 30% without sacrificing the foods you love."' ],
                        ...
                        'featured_image_gen' => [ 'type' => 'boolean', 'description' => 'Trigger async AI featured image generation. Leave enabled unless the user is supplying their own image via upload_image.', 'default' => true ],
                    ],
                ],
            ],
    ...
    private function tool_generate_post( array $args, int $actor_id ): array {
        $post_id = ILLE_PG_Post_Creator::create( [
            ...
            'featured_image' => filter_var( $args['featured_image_gen'] ?? true, FILTER_VALIDATE_BOOLEAN ),
